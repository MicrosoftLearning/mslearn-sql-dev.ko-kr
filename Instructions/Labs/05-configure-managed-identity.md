...
1. **만들기**를 선택하고 필요한 세부 정보를 입력합니다.
    ...
    | **프로젝트 세부 정보** | **지역** | 웹앱을 호스팅할 지역을 선택합니다. |
    | **웹앱 세부 정보** | **이름** | 웹앱에 대한 고유한 이름을 입력합니다. |
    | **웹앱 세부 정보** | **런타임 스택** | .NET 8(LTS) |
    | **데이터베이스** | **엔진** | SQLAzure |
...
1. Azure Data Studio에서 데이터베이스에 연결하고 다음 코드를 실행합니다.
...
1. Azure Data Studio를 사용하여 Azure SQL Database에 연결 **Microsoft Entra ID - MFA 지원이 있는 유니버설**을 선택하고 사용자 이름을 입력합니다.
